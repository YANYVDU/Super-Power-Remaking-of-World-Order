...
function RefreshYourReligion()
	...
	if (eReligion > ReligionTypes.RELIGION_PANTHEON) then
		Controls.YourReligionFollowers:SetText(Locale.Lookup("TXT_KEY_RO_YOUR_FOLLOWERS", Game.GetNumFollowers(eReligion)));
		Controls.YourReligionCities:SetText(Locale.Lookup("TXT_KEY_RO_YOUR_CITIES", Game.GetNumCitiesFollowing(eReligion)));
		Controls.YourReligionFollowers:SetHide(false);
		Controls.YourReligionCities:SetHide(false);
	else
		Controls.YourReligionFollowers:SetHide(true);
		Controls.YourReligionCities:SetHide(true);
	end
	...
	local holyCity = nil;
	if eReligion > ReligionTypes.RELIGION_PANTHEON then
		local hc = Game.GetHolyCityForReligion(eReligion, player:GetID());
		if hc and hc:GetOwner() == player:GetID() then holyCity = hc; end
	end
	if(capital ~= nil) then
		for unit in GameInfo.Units() do
			if player:IsCanPurchaseAnyCity(false, true, unit.ID, -1, YieldTypes.YIELD_FAITH) then
				if (player:DoesUnitPassFaithPurchaseCheck(unit.ID)) then
					local faithCost = capital:GetUnitFaithPurchaseCost(unit.ID, true);
					if faithCost <= 0 and holyCity then
						faithCost = holyCity:GetUnitFaithPurchaseCost(unit.ID, true);
					end
					if faithCost > 0 then
		...
		for building in GameInfo.Buildings() do
			if player:IsCanPurchaseAnyCity(false, true, -1, building.ID, YieldTypes.YIELD_FAITH) then
				local faithCost = capital:GetBuildingFaithPurchaseCost(building.ID);
				if faithCost <= 0 and holyCity then
					faithCost = holyCity:GetBuildingFaithPurchaseCost(building.ID);
				end
				if faithCost > 0 then
