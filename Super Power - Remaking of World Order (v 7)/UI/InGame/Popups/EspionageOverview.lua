...
-- Super Power V11: append the per-turn steal tech (gathering intel) breakdown to the activity tooltip.
-- Each non-zero modifier is listed on its own line, mirroring the source calculation in CalcGatheringIntelPerTurn.
local function GetGatheringIntelTooltip(pPlayer, agent, city)
	local strTT = "";
	if (city == nil) then
		return strTT;
	end

	local breakdown = pPlayer:GetEspionageGatheringIntelInfo(agent.AgentID);
	if (breakdown == nil) then
		return strTT;
	end

	strTT = strTT .. "[NEWLINE]";
	strTT = strTT .. Locale.ConvertTextKey("TXT_KEY_EO_SPY_GATHERING_INTEL_BASE_TT", math.floor(breakdown.iBaseRate / 100));

	local function appendModifier(szTag, iValue)
		if (iValue ~= 0) then
			local strValue = "";
			if (iValue > 0) then
				strValue = "[COLOR_POSITIVE_TEXT]+" .. iValue .. "%[ENDCOLOR]";
			else
				strValue = "[COLOR_NEGATIVE_TEXT]" .. iValue .. "%[ENDCOLOR]";
			end
			strTT = strTT .. "[NEWLINE]";
			strTT = strTT .. Locale.ConvertTextKey(szTag, strValue);
		end
	end

	appendModifier("TXT_KEY_EO_SPY_GATHERING_INTEL_CITY_TT", breakdown.iCityMod);
	appendModifier("TXT_KEY_EO_SPY_GATHERING_INTEL_BUILDING_TT", breakdown.iPlayerMod);
	appendModifier("TXT_KEY_EO_SPY_GATHERING_INTEL_THEIR_POLICY_TT", breakdown.iTheirPolicyMod);
	appendModifier("TXT_KEY_EO_SPY_GATHERING_INTEL_MY_POLICY_TT", breakdown.iMyPolicyMod);
	appendModifier("TXT_KEY_EO_SPY_GATHERING_INTEL_CSUA_TT", breakdown.iCSUAMod);
	appendModifier("TXT_KEY_EO_SPY_GATHERING_INTEL_RANK_TT", breakdown.iSpyRankMod);
	appendModifier("TXT_KEY_EO_SPY_GATHERING_INTEL_SPEED_TT", breakdown.iSpeedMod);
...
function RelocateAgent(agentID, city)
	...
	elseif (agent.AgentActivity == Locale.Lookup("TXT_KEY_SPY_STATE_GATHERING_INTEL")) then
		...
		strActivityTT = strActivityTT .. GetGatheringIntelTooltip(activePlayer, agent, city);	
	...
	for i,v in ipairs(agents) do
		...
		if(v.State == "TXT_KEY_SPY_STATE_DEAD") then
			...
			elseif (v.AgentActivity == Locale.Lookup("TXT_KEY_SPY_STATE_GATHERING_INTEL")) then
				...
				strActivityTT = strActivityTT .. GetGatheringIntelTooltip(pActivePlayer, v, city);	
