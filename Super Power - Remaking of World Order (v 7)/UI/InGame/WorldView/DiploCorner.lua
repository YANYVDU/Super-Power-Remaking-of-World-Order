...
function DoUpdateEspionageButton()
	...
	local iSpyPoints = pLocalPlayer:GetSpyPoints();
	local iSpyPointsThreshold = pLocalPlayer:GetSpyPointsThreshold();
	local iSpyPointsPerTurn = pLocalPlayer:GetSpyPointsPerTurn();
	...
	-- Super Power V11: show Spy Points progress toward the next spy.
	if (iSpyPointsThreshold > 0) then
		strToolTip = strToolTip .. "[NEWLINE][NEWLINE]";
		strToolTip = strToolTip .. Locale.ConvertTextKey("TXT_KEY_SPY_POINTS_PROGRESS_TT", iSpyPoints, iSpyPointsThreshold, iSpyPointsPerTurn);
	end
