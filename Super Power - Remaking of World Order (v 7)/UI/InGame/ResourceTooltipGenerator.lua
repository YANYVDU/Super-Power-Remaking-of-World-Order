...
-- SP: cache luxury era-decay info (resource Type -> era Description key), built once via DB.Query (faster than GameInfo per call)
local g_ResourceDecayEra = {};
for row in DB.Query("SELECT r.Type, e.Description FROM Resources r, Eras e WHERE r.HappinessDecayEra = e.Type AND r.HappinessDecayEra IS NOT NULL AND r.HappinessDecayEra != ''") do
    g_ResourceDecayEra[row.Type] = row.Description;
end
...
function GenerateResourceToolTip(plot)
	...
	-- SP: Luxury era decay hint (from the DB.Query-built cache above)
	local strEraDesc = g_ResourceDecayEra[pResourceInfo.Type];
	if (strEraDesc ~= nil) then
		local strEraName = Locale.ConvertTextKey(strEraDesc);
		strToolTip = strToolTip .. "[NEWLINE][ICON_BULLET][COLOR_WARNING_TEXT]" .. Locale.ConvertTextKey("TXT_KEY_RESOURCE_TOOLTIP_ERA_DECAY", strEraName) .. "[ENDCOLOR]";
	end
