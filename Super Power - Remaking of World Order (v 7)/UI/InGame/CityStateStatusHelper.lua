...
local function GetPosInfRange(iMajor)
	...
	if pMajor and pMajor.GetMinorCivAlliesThreshold then
		return math.abs(pMajor:GetMinorCivAlliesThreshold() - GameDefines["FRIENDSHIP_THRESHOLD_NEUTRAL"]);
	end
...
function GetCityStateUAHelpText(iMajor, iMinor)
	...
	if GameInfo.CityStateUAs == nil or GameInfo.CityStateUAEffects == nil then
		return "";
	end
	local minorCivInfo = GameInfo.MinorCivilizations[pMinor:GetMinorCivType()];
	if minorCivInfo == nil then
		return "";
	end
	local uaType = minorCivInfo.UAType;
	if uaType == nil or uaType == "" then
		return "";
	end
	local uaInfo = GameInfo.CityStateUAs[uaType];
	if uaInfo == nil then
		return "";
	end
	...
	if uaInfo.AllyEffectType ~= nil and uaInfo.AllyEffectType ~= "" then
		local allyEffect = GameInfo.CityStateUAEffects[uaInfo.AllyEffectType];
		if allyEffect ~= nil and allyEffect.Help ~= nil and allyEffect.Help ~= "" then
			strAllyHelp = Locale.ConvertTextKey(allyEffect.Help);
		end
	end

	if uaInfo.FriendEffectType ~= nil and uaInfo.FriendEffectType ~= "" then
		local friendEffect = GameInfo.CityStateUAEffects[uaInfo.FriendEffectType];
		if friendEffect ~= nil and friendEffect.Help ~= nil and friendEffect.Help ~= "" then
			strFriendHelp = Locale.ConvertTextKey(friendEffect.Help);
		end
	end
	...
	if pMajor ~= nil then
		if pMinor:IsAllies(iMajor) then
			strAllyHelp = "[COLOR_POSITIVE_TEXT]" .. strAllyHelp .. "[ENDCOLOR]";
		elseif pMinor:IsFriends(iMajor) then
			strFriendHelp = "[COLOR_POSITIVE_TEXT]" .. strFriendHelp .. "[ENDCOLOR]";
		end
	end
	...
	if strFriendHelp ~= "" then
		if result ~= "" then
			result = result .. "[NEWLINE][NEWLINE]";
		end
		result = result .. strFriendHelp;
	end
...
function GetCityStateStatusRow(iMajor, iMinor)
	...
	-- Friends
	elseif (iInf < pMajor:GetMinorCivAlliesThreshold()) then
...
function UpdateCityStateStatusBar(iMajor, iMinor, posBarCtrl, negBarCtrl, barMarkerCtrl)
	...
	if (info.PositiveStatusMeter ~= nil) then
		local posInfRange = GetPosInfRange(iMajor);
		local percentFull = math.abs(iInf) / posInfRange;
...
function GetCityStateStatusToolTip(iMajor, iMinor, bFullInfo)
	...
	elseif (pMinor:IsFriends(iMajor)) then		-- Friends
		...
		local strTempTT = Locale.ConvertTextKey("TXT_KEY_FRIENDS_CSTATE_TT", strShortDescKey, iAlliesThreshold);
